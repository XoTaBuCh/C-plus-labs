...
private:
    struct Node {
        ...
        Node* pPrev;
        T data;
        Node(T data = T(), Node* pNext = NULL, Node* pPrev = NULL) { // REVIEW nullptr вместо NULL
            this->pNext = pNext;
            this->pPrev = pPrev;
            this->data = data;
        }
...
/* REVIEW
 * leak'и остаются, бонуса нет за это
 * есть (будут) бонус за шаблоны
 * ломается копирование
 List<int> arr = {1, 2, 3};
 List<int> arr2 = arr;
 * когда починишь, то должно быть так
 *(++arr2.begin()) = 0;
 ...
 * 3 == *(--arr.end()) оно работает но пусть не поломается))0)
 * 1 == *arr.begin() оно работает но пусть не поломается))0)
 * надо чуть доработать чтоб я мог хотя бы потестить
 * почти зачтено
 */
